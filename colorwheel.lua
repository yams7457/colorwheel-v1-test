...
function restart_sequences()
  for i = 1,4,1 do
    params:set("current gate step " ..i, params:get("gate sequence end " ..i))
    params:set("current interval step " ..i, params:get("interval sequence end " ..i))
    params:set("current octave step " ..i, params:get("octave sequence end " ..i))
    params:set("current velocity step " ..i, params:get("velocity sequence end " ..i))
    params:set("current length step " ..i, params:get("length sequence end " ..i))
  end
...
function g.key(x,y,z)
  ...
  if navigation_bar.sequence_or_live == 0 then -- this is all for the sequencer pages!
      ...
      if y == 8 then -- this is all for the seq nav bar!
        ...
        elseif x == 14 then
          if momentary_prob == false and z == 1 then
            momentary_prob = true
              if momentary_time == true then
                g.key(13,8,1)
              end
          elseif momentary_prob == true and z == 1 then
            momentary_prob = false
          end
        end
     end
  ...
  if not momentary_time and not momentary_prob then -- if no mod keys are pressed
    ...
    if momentary_alt and navigation_bar.displayed_trait == 1 and y <= 4 then
    ...
    elseif z == 1 then
      if navigation_bar.displayed_trait == 1 then -- this is all for the 'traits' pages
        ...
        if y == 8 and x >= 13 and x <= 14 then
          activate_modifier(x,y,z)
        end

      elseif navigation_bar.displayed_trait == 2 then
        if y <= 5 then
          if 6 - y == params:get('interval ' ..navigation_bar.displayed_track..  ' ' ..x) then
             gate_toggle(x,navigation_bar.displayed_track,z)
            else
  ...
  if navigation_bar.sequence_or_live == 1 then
    ...
    if y == 7 and x == 2 and z == 1 then
      params:set('gate and note sync', params:get('gate and note sync') % 2 + 1)
    end
...
function set_up_the_nav_bar()
  ...
  for i = 1,16,1 do
    ...
    if momentary_prob then
      ...
      g:led(navigation_bar.displayed_track, 8, 8)
      if navigation_bar.displayed_trait <= 5 then
      g:led(navigation_bar.displayed_trait + 5, 8, 8)
      elseif navigation_bar.displayed_trait > 5 then
...
function set_up_the_live_page(x,y,z)
  for i = 1,4 do
    ...
    g:led(2, 7, params:get('gate and note sync') * 4)
...
if not momentary_prob and not momentary_time then -- if no mod keys are pressed
  ...
  g:led(params:get("global clock div"), 7, 12)
    ...
    g:led(params:get('current gate step ' ..y), y, 15)
    end
  end
end
end
...
function set_up_the_octave_page(track)
  if not momentary_prob and not momentary_time then -- if no mod keys are pressed
    ...
    g:led(params:get('current octave step ' ..track), 6, 15)
    g:led(params:get('octave div ' ..track), 7, 4)
    end
...
if not momentary_prob and not momentary_time then -- if no mod keys are pressed
    ...
    g:led(params:get('current velocity step ' ..track), 6, 15)
    g:led(params:get('velocity div ' ..track), 7, 4)
      end
...
function set_up_the_length_page(track)
  if not momentary_prob and not momentary_time then -- if no mod keys are pressed
   local k = {}
    ...
    g:led(params:get('current length step ' ..track), 6, 15)
    g:led(params:get('length div ' ..track), 7, 4)
      end
...
function set_up_the_alt_note_page(track)
    if not momentary_prob and not momentary_time then -- if no mod keys are pressed
      for x = 1,16,1 do
        g:led(x, 5 - params:get('alt note ' ..track.. ' ' ..x), 2)
      end
    ...
    for x = params:get('alt note sequence start ' ..track), params:get('alt note sequence end ' ..track) do
      g:led(x, 6, 4)
        ...
        g:led(params:get('current alt note step ' ..track), 6, 12)
      end
...
function change_start_point(x,y,z,trait,track)
   if z == 1 then
     ...
     print(params:get('gate sequence start ' ..track))
     end
  end
    if z == 0 then
  range[trait][track].held = range[trait][track].held - 1
  ...
  if trait == 2 then
    params:set('gate sequence start ' ..track, params:get('interval sequence start ' ..track))
    params:set('gate sequence end ' ..track, params:get('interval sequence end ' ..track))
  end

  if trait == 1 then
    params:set('interval sequence start ' ..track, params:get('gate sequence start ' ..track))
    params:set('interval sequence end ' ..track, params:get('gate sequence end ' ..track))
  end
